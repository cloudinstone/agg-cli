...
pub fn antigravity_db_path() -> PathBuf {
    dirs::home_dir().unwrap().join("Library/Application Support/Antigravity/User/globalStorage/state.vscdb")
}
...
pub fn clear_antigravity_auth_state() {
    ...
    if let Ok(conn) = rusqlite::Connection::open(&db_path) {
        let _ = conn.execute("DELETE FROM itemTable WHERE key LIKE 'antigravityUnifiedStateSync.oauthToken%'", []);
        let _ = conn.execute("DELETE FROM itemTable WHERE key LIKE 'antigravityUnifiedStateSync.userStatus%'", []);
        let _ = conn.execute("DELETE FROM itemTable WHERE key LIKE 'secret://aicoding.auth%'", []);
    ...
    if was_running {
        ...
        if is_ide_running() {
            ...
            for pattern in helper_patterns {
                let _ = std::process::Command::new("pkill").arg("-9").arg("-f").arg(pattern).output();
            }
        }
    }
    was_running
...
pub fn get_current_ide_email() -> Option<String> {
    ...
    if fs::copy(&db_path, &temp_db).is_err() { return None; }
    let conn = rusqlite::Connection::open(&temp_db).ok()?;
    let mut stmt = conn.prepare("SELECT value FROM itemTable WHERE key = 'antigravityUnifiedStateSync.userStatus'").ok()?;
