...
use webbrowser;
use tiny_http;
use crate::modules::config::{AppConfig, AccountInternal, save_account};
use crate::modules::api::{refresh_access_token, fetch_user_info, fetch_account_metadata, fetch_account_model_data, fetch_email_from_api, fetch_user_plan_info, fetch_credit_usage_info, build_auth_url, build_oauth_blob, build_status_blob, oauth_client_config, TokenResponse, TOKEN_URL};
use crate::modules::db::{antigravity_db_path, apply_account_session, clear_antigravity_auth_state, get_current_ide_email, is_ide_running, terminate_ide, restart_ide};
...
fn print_account_group(label_index: Option<usize>, display_name: &str, group: &AccountGroup) {
    let active_marker = if group.is_active { "\x1b[1;32m● [ACTIVE]\x1b[0m" } else { "\x1b[90m○ [IDLE]\x1b[0m" };
    let tier_color = if group.tier == "PRO" { "\x1b[1;35m" } else { "\x1b[1;36m" };
    ...
    println!("{prefix}{} {}   {}", tier_label, display_name, active_marker);
    ...
    if !group.models.is_empty() {
        ...
        for m in &group.models {
            let pct = if m.usage == "N/A" { 0.0 } else { m.usage.replace("%", "").parse::<f64>().unwrap_or(0.0) };
            let bar_len = 10;
            let filled = (pct / 100.0 * bar_len as f64).round() as usize;
            let bar = format!("\x1b[32m{}\x1b[90m{}\x1b[0m", "█".repeat(filled), "█".repeat(bar_len - filled));
...
async fn perform_add_or_login(config: &mut AppConfig, refresh: Option<String>, capture: bool, alias: Option<String>, switch: bool) {
    if capture {
        let email = get_current_ide_email();
        if let Some(em) = email {
            let db_path = antigravity_db_path();
            let temp_db = std::env::temp_dir().join("aag_add.db");
            if let Ok(_) = fs::copy(&db_path, &temp_db) {
                if let Ok(conn) = rusqlite::Connection::open(&temp_db) {
                    ...
                    println!("✅ Successfully captured and saved account from IDE: {}{}", em, if let Some(a) = alias { format!(" (Alias: {})", a) } else { "".to_string() });
                    return;
                }
            }
        }
        println!("❌ Could not detect a valid Email from current IDE. Please log in to the IDE first.");
        return;
    }
    ...
    if let Some(rf) = refresh {
        ...
        if let Some(token) = refresh_access_token(&rf).await {
            if let Some(user_info) = fetch_user_info(&token.access_token).await {
                if let (Some(email), Some(status_blob)) = (user_info.email.clone(), build_status_blob(&user_info)) {
    ...
    if let Ok(server) = tiny_http::Server::from_listener(listener, None) {
        if let Some(request) = server.incoming_requests().next() {
            ...
            if let Some(c) = url.query_pairs().find(|(k, _)| k == "code").map(|(_, v)| v.into_owned()) {
                let _ = request.respond(tiny_http::Response::from_string("<h1>Login successful, please return to the terminal.</h1>"));
                let client = reqwest::Client::new();
                let body = format!(
                    "client_id={}&client_secret={}&code={}&grant_type=authorization_code&redirect_uri={}",
                    oauth.client_id, oauth.client_secret, c, redirect_uri
                );
                let res = client.post(TOKEN_URL).header("Content-Type", "application/x-www-form-urlencoded").body(body).send().await;
                if let Ok(r) = res {
                    if let Ok(tr) = r.json::<TokenResponse>().await {
                        if let Some(refresh) = tr.refresh_token {
                            if let Some(user_info) = fetch_user_info(&tr.access_token).await {
                                if let (Some(email), Some(status_blob)) = (user_info.email.clone(), build_status_blob(&user_info)) {
                                    let token_blob = build_oauth_blob(&tr.access_token, &refresh, tr.expires_in);
                                    let account = AccountInternal {
                                        alias: alias.clone(),
                                        email: Some(email.clone()),
                                        token_blob: token_blob.clone(),
                                    ...
                                    if switch {
                                        let user_plan = fetch_user_plan_info(&tr.access_token).await;
                                        let project_id = user_plan.get("cloudaicompanionProject")
                                            .and_then(|p| p.as_str().map(|s| s.to_string()).or_else(|| p.get("id").and_then(|id| id.as_str()).map(|s| s.to_string())));
                                        let credit_usage = if let Some(pid) = project_id {
                                            fetch_credit_usage_info(&tr.access_token, &pid).await
                                        } else {
                                            serde_json::json!({})
                                        };
                                        let user_info_json = serde_json::to_value(&user_info).unwrap_or(serde_json::json!({}));

                                        if apply_account_session(&token_blob, &status_blob, &user_info_json, &user_plan, &credit_usage) {
                                            terminate_ide();
...
async fn main() {
    ...
    match cli.command {
        Commands::List { refresh } => {
            if config.accounts.is_empty() { println!("💡 Please use `aag login` first."); return; }
            ...
            for (idx, acc) in config.accounts.iter().enumerate() {
                ...
                tasks.push(async move {
                    let mut email = email_cached.clone().unwrap_or_else(|| "Unknown".to_string());
                    ...
                    if let Some(rf) = rf_token {
                        if let Some(token) = refresh_access_token(&rf).await {
                            if email == "Unknown" { if let Some(e) = fetch_email_from_api(&token.access_token).await { email = e; } }
                            if let Some(meta) = fetch_account_metadata(&token.access_token).await {
                                tier = meta.tier;
                                if meta.validation_required { status_detail = Some("Needs validation (Verify on device)".to_string()); }
                                let project_id = meta.project_id.as_deref().unwrap_or("");
                                let (m_rows, _) = fetch_account_model_data(&token.access_token, project_id, &email, force_refresh).await;
                                models = m_rows;
                            } else { error = Some("Handshake Fail".into()); }
                        } else { error = Some("Refresh Fail".into()); }
                    } else { status_detail = Some("Session-only account".into()); }

                    let is_active = active_em.as_ref() == Some(&email);
                    (idx, AccountGroup { email, is_active, tier, status_detail, models, error })
                });
            }
            
            let mut results = join_all(tasks).await;
            results.sort_by(|(_, a), (_, b)| {
                let a_available = a.status_detail.as_deref() != Some("Needs validation (Verify on device)");
                let b_available = b.status_detail.as_deref() != Some("Needs validation (Verify on device)");
        ...
        Commands::Status { internal } => {
            ...
            let Some(acc) = config.accounts.iter().find(|a| a.email.as_deref() == Some(em.as_str())) else {
                ...
                if let Some(token) = refresh_access_token(rf).await {
                    if let Some(meta) = fetch_account_metadata(&token.access_token).await {
                        ...
                        if meta.validation_required {
                            group.status_detail = Some("Needs validation (Verify on device)".to_string());
                        }
                        let project_id = meta.project_id.unwrap_or_default();
                        let (models, _) = fetch_account_model_data(&token.access_token, &project_id, &em, false).await;
                        group.models = models;
                        if internal {
                            raw_plan = Some(fetch_user_plan_info(&token.access_token).await);
                            raw_models = Some(fetch_credit_usage_info(&token.access_token, &project_id).await);
                        }
                    } else {
            ...
            if internal {
                if let (Some(plan), Some(models)) = (raw_plan, raw_models) {
                    ...
                    println!("{}", serde_json::to_string_pretty(&models).unwrap_or_else(|_| "{}".to_string()));
                } else {
                    println!("\nℹ️ `--internal` requires a saved refresh token for the current account.");
                }
            }
        }
        Commands::Add { alias, refresh, capture, switch } => {
            perform_add_or_login(&mut config, refresh, capture, alias, switch).await;
        }
        Commands::Login { alias, refresh, capture } => {
        ...
        Commands::Logout => {
            ...
            if was_running { restart_ide(); }
        }
        Commands::Switch { email_or_index } => {
            if config.accounts.is_empty() { println!("💡 No accounts available. Please use `aag login`."); return; }
            ...
            for (idx, acc) in config.accounts.iter().enumerate() {
                ...
                tasks.push(tokio::spawn(async move {
                    ...
                    let mut display_email = acc.email.clone().unwrap_or_else(|| "Unknown".to_string());
                    ...
                    if let Some(rf) = &acc.refresh_token {
                        if let Some(token) = refresh_access_token(rf).await {
                            if let Some(meta) = fetch_account_metadata(&token.access_token).await {
                                ...
                                let project_id = meta.project_id.as_deref().unwrap_or("");
                                let (models, _) = fetch_account_model_data(&token.access_token, project_id, &user_email, true).await;
                                for m in models {
                                    if m.id == "gemini-3.1-pro-high" { gemini_quota = m.usage; }
                                    else if m.id == "claude-opus-4-6-thinking" { claude_quota = m.usage; }
                                }
                            } else { error = Some("Handshake Fail".to_string()); }
                        } else { error = Some("Token Expired".to_string()); }
                    } else {
                        tier = "SESSION".to_string();
                    }

                    (idx, display_email, tier, needs_validation, error, gemini_quota, claude_quota)
                }));
            }
            ...
            for res in results {
                if let Ok((idx, email, tier, needs_val, err, gq, cq)) = res {
                    ...
                    items_data.push(MenuItem {
                        idx,
                        display_name: if let Some(alias) = config.accounts[idx].alias.as_ref() { format!("{} ({})", alias, email) } else { email.clone() },
                        gemini_quota: gq,
                        claude_quota: cq,
            ...
            let menu_items: Vec<String> = items_data.iter()
                .map(|item| {
                    let format_quota = |label: &str, q: &str| {
                        let q_val = if q == "N/A" { " N/A" } else { q };
                        let color = if q == "N/A" { "\x1b[90m" } 
                                   else if q.replace("%","").parse::<f64>().unwrap_or(0.0) > 50.0 { "\x1b[32m" } 
                                   else { "\x1b[31m" };
                        
                        let label_color = if label == "G" { "\x1b[1;36m" } else { "\x1b[1;33m" };
                        format!("{}{}\x1b[0m {}{:>4}\x1b[0m", label_color, label, color, q_val)
                    };
                    ...
                    let tier_part = if item.tier == "SESSION" {
                        ...
                        let tier_color = if item.tier == "PRO" { "\x1b[1;35m" } else { "\x1b[1;36m" };
            ...
            let acc_index = if let Some(input) = email_or_index {
                if input.starts_with('#') {
                    let idx = input[1..].parse::<usize>().unwrap_or(0);
                    if idx > 0 && idx <= config.accounts.len() { idx - 1 }
                    else { println!("❌ Invalid index"); return; }
                } else {
                    match config.accounts.iter().position(|a| a.email.as_ref() == Some(&input) || a.alias.as_ref() == Some(&input)) {
                        Some(i) => i,
                        None => { println!("❌ Could not find account '{}'", input); return; }
                    }
                }
            } else {
                match Select::with_theme(&ColorfulTheme::default()).with_prompt("Please select an account to switch to").default(0).items(&menu_items).interact_opt() {
                    Ok(Some(sel_idx)) => index_mapping[sel_idx],
                    Ok(None) => { println!("❌ Cancelled."); return; }
            ...
            println!("🚀 Switching to account: {}...", resolved_email.as_deref().unwrap_or("Unknown"));
            ...
            if let Some(refresh_token) = &acc.refresh_token {
                if let Some(token) = refresh_access_token(refresh_token).await {
                    token_blob = build_oauth_blob(&token.access_token, refresh_token, token.expires_in);
                    if let Some(user_info) = fetch_user_info(&token.access_token).await {
                        ...
                        if apply_account_session(&token_blob, &status_blob, &user_info_json, &user_plan, &credit_usage) {
                            if let Some(account) = config.accounts.get_mut(acc_index) {
                                ...
                                if resolved_email.is_some() { account.email = resolved_email.clone(); }
                            }
                            config.save();
                            
                            if was_running {
                                let restarted = terminate_ide();
                                if restarted { restart_ide(); }
                            }
                ...
                if apply_account_session(&token_blob, &status_blob, &serde_json::json!({}), &serde_json::json!({}), &serde_json::json!({})) {
                    if was_running {
                        let restarted = terminate_ide();
                        if restarted { restart_ide(); }
                    }
                ...
                eprintln!("❌ Failed to refresh this account and could not write cached session data.");
                return;
            }
            ...
            } else { eprintln!("❌ Failed to write login state."); }
        }
        Commands::Remove { email } => {
            if let Some(e) = email {
                let original_len = config.accounts.len();
                config.accounts.retain(|a| a.email.as_ref() != Some(&e) && a.alias.as_ref() != Some(&e));
                if config.accounts.len() < original_len { config.save(); println!("✅ Deleted account: {}", e); }
                else { println!("❌ Could not find account: {}", e); }
            } else {
                if config.accounts.is_empty() { println!("💡 No accounts available."); return; }
                let items: Vec<String> = config.accounts.iter().map(|acc| acc.email.as_deref().unwrap_or("Unknown Email (Run `aag clean`)").to_string()).collect();
                match Select::with_theme(&ColorfulTheme::default()).with_prompt("Please select an account to delete").default(0).items(&items).interact_opt() {
                    Ok(Some(index)) => { let removed = config.accounts.remove(index); config.save(); println!("✅ Deleted account: {}", removed.email.unwrap_or_else(|| "Unknown".to_string())); },
                    Ok(None) => { println!("❌ Cancelled."); return; }
                    Err(_) => { println!("❌ Cancelled."); return; }
                }
            }
        ...
        Commands::Clean => {
            ...
            for acc in config.accounts.into_iter().rev() {
                if let Some(ref email) = acc.email {
                    if !seen.contains(email) { seen.insert(email.clone()); unique_accounts.push(acc); }
                }
            }
            config.accounts = unique_accounts.into_iter().rev().collect();
            let removed_count = original_len - config.accounts.len();
            if removed_count > 0 { config.save(); println!("🧹 Cleanup complete! Removed {} invalid or duplicate entries.", removed_count); }
            else { println!("✨ List is already clean."); }
        }
    }
}
